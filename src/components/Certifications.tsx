      ...
      <div className="w-[90%] mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {certifications.map((cert, idx) => (
            ...
            >
              <div>
                <div className="relative h-44 rounded-2xl overflow-hidden mb-6 bg-slate-950 border border-white/10">
                  <img
                    src={cert.badgeImage}
                    alt={cert.title}
                  ...
                  <div className="absolute top-3 right-3 bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 text-[10px] font-bold px-2.5 py-1 rounded-full flex items-center gap-1 backdrop-blur-md">
                    <ShieldCheck className="w-3 h-3 text-indigo-400" />
                    <span>Verified</span>
                  </div>
                </div>

                <div className="text-[11px] font-mono font-bold text-indigo-400 mb-1">
                  {cert.issuer} • {cert.issueDate}
                </div>

                <h3 className="text-lg font-bold text-white font-display mb-3 group-hover:text-indigo-400 transition-colors">
                  {cert.title}
                </h3>
              </div>
              ...
              <div>
                <div className="flex flex-wrap gap-1.5 pt-4 border-t border-white/5">
                  ...
                  {(cert.skillsCovered || []).length > 3 && (
                    <span className="px-1.5 py-0.5 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-[10px] text-indigo-300 font-bold">
                      +{cert.skillsCovered.length - 3}
                    </span>
                  )}
                </div>
