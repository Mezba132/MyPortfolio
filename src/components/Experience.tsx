            ...
            {experiences.map((exp, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute left-4 sm:left-1/2 top-6 -translate-x-1/2 w-8 h-8 rounded-full bg-[#03060d] border-2 border-indigo-500 shadow-lg shadow-indigo-500/30 flex items-center justify-center z-20 group-hover:scale-110 transition-transform">
                  ...
                  <div className={`sm:w-1/2 ${isEven ? 'sm:pr-10 sm:text-right' : 'sm:ml-auto sm:pl-10 sm:text-left'} pl-12 sm:pl-0`}>
                    <div
                      ...
                      className={`p-6 sm:p-7 rounded-3xl bg-[#0a0f1d] border transition-all duration-300 cursor-pointer shadow-2xl backdrop-blur-md ${
                        isExpanded
                    ...
                    >
                      {/* Top Meta info */}
                      <div className={`flex items-center gap-2 mb-2 text-xs font-bold font-mono text-indigo-400 ${isEven ? 'sm:justify-end' : 'justify-start'}`}>
                        {exp.current && (
                          <span className="px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-bold uppercase animate-pulse">
                            Current Role
                          </span>
                        )}
                      ...
                      <div className={`flex items-center gap-2 text-xs font-semibold text-indigo-300 mb-4 ${isEven ? 'sm:justify-end' : 'justify-start'}`}>
                      ...
                      <div className={`flex flex-wrap gap-1.5 mb-4 ${isEven ? 'sm:justify-end' : 'justify-start'}`}>
                        {(exp.technologies || []).slice(0, 5).map((tech, i) => (
                          <span
                            key={i}
                            className="px-2.5 py-1 rounded-xl bg-white/5 border border-white/10 text-[11px] font-medium text-slate-300"
                          >
                            {tech}
                          </span>
                        ))}
                        {(exp.technologies || []).length > 5 && (
                          <span className="px-2 py-1 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-[11px] font-bold text-indigo-300">
                            +{exp.technologies.length - 5}
                          </span>
                        )}
                      </div>
                      ...
                      {/* Expand Button */}
                      <div className={`flex items-center gap-1 text-xs font-bold text-indigo-400 hover:text-indigo-300 ${isEven ? 'sm:justify-end' : 'justify-start'}`}>
                      ...
                      <AnimatePresence>
                        {isExpanded && (
                          <motion.div
                            ...
                            className="pt-5 mt-5 border-t border-white/10 text-left space-y-4"
                          >
